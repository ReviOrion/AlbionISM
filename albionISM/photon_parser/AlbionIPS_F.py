...
from parser import PhotonParser
from messages import EventData, OperationRequest, OperationResponse
import packet_classes.packet_deserializer as pd
...
class AlbionPacketSniffer:
    def __init__(self, save_format='json', save_directory='captured_packets', interface=None):
        ...
        self.albion_ports = [5056, 5055, 5054, 5053, 5052]  # 5054 - Login screen, 5056 - events, 4535 - legacy (test servers)
        ...
        self.photon_parser = PhotonParser(self.on_ev, self.on_rq, self.on_rs)
    
    def on_ev(self, payload: EventData, timestamp: Optional[int] = None, _: Optional[int] = None) -> None:
        if 252 in payload.parameters:
            
            code: int = payload.parameters[252]
            #print(payload.parameters)
            match code:
                case 99: #Attach items to container on open
                    #print(payload.parameters)
                    packet = pd.packet_99_attachItemContainer()
                    packet.try_deserialize(payload.parameters)
                    packet.pretty_print()
                case 100: #Detach item container
                ...
                case 32: # simple items (food etc.)
                    ...
                    case 32: # simple items (food etc.)
                    print(payload.parameters) 
                    case 101: #Invalidate item container
                        print(payload.parameters)
                    case 102: #Lock item container
                        print(payload.parameters)
                    case 26: #InventoryPutItem
                        print(payload.parameters)
                    case 27: #InventoryDeleteItem
                        print(payload.parameters)
    ...
    def packet_callback(self, packet: Optional[Callable[[Packet], Any]]):
        if packet is not None:
            if Raw in packet:
                try:
                    self.photon_parser.handle_payload(packet.load)
                except Exception as error:
                    traceback.print_exception(type(error), error, error.__traceback__)
        else: print("packet is null")
    ...
    def start_capture(self):
        ...
        #        
        #    )
            bpf_filter = " or ".join([f"tcp port {port} || udp port {port}" for port in [5054, 5055, 5056, 5057, 4535]])
            sniff(prn=self.packet_callback, store=False, filter=bpf_filter)
